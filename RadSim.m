...
for n=0:delt:10
    ...
    velocitynext=[delt*(q/m*(vy*Bz-vz*By))+vx,delt*(q/m*(vz*Bx-vx*Bz))+vy,delt*(q/m*(vx*By-vy*Bx))+vz];
    positionnext=[velocitynext(1)*delt/2+px,velocitynext(2)*delt/2+py,velocitynext(3)*delt+pz];
  ...
  if (positionnext(3)>=-innerradius) && (positionnext(3)<=innerradius)
    ...
    if abs(positionnext(1))>abs(scale*totalradius) || abs(positionnext(2))>abs(scale*totalradius) || abs(positionnext(3))>abs(scale*totalradius)
